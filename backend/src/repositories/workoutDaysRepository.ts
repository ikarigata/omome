...
export function createWorkoutDaysRepository(db: DB) {
  const repo = {
    ...
    async insert(data: { id: string; userId: string; date: string; title?: string; notes?: string }) {
      try {
        const [row] = await db
          .insert(workoutDays)
          .values({
    ...
    async update(
      id: string,
      data: { date?: string; title?: string | null; notes?: string | null },
    ) {
      const [row] = await db.update(workoutDays).set(data).where(eq(workoutDays.id, id)).returning()
      return row ?? null
    },
...
export type WorkoutDaysRepository = ReturnType<typeof createWorkoutDaysRepository>
